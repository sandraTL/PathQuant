...
PathQuant aims to quantify associations between gene-metabolite associations in a selected KEGG pathway.
To do so, PathQuant calculates distances, which are define as the number of reaction separating a gene (represented as the edge of the pathway) and a metabolite (node of the pathway) We developped functions to give the user tools to calculate dsitance for associatiosn data, but also to be able to compare the distance of any gene-metabolite pairs.
...
####Arguments : 
    pathwayId: KEGG Id of selected pathway.
    association :Dataframe with 2 columns, where each line reprensents an
                           associations. First column are genes and the sencond                                  column as the metabolites. Only use KEGG Ids.
    completeMetaboliteDF : is a dataframe of 1 column with the KEGG Ids of
                           all measured metabolites.


```{r, size=12}
df1 <- getDistanceAll("hsa01100", shinAndAlDF, completeMetaboDF)
df1[1:5,]
```
